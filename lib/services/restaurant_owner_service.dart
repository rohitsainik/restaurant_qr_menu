...
class RestaurantOwnerService extends BaseService<RestaurantModel> {
  ...
  Stream<List<RestaurantModel>> getRestaurantData() {
    return ref!
        .where(Restaurants.userId, isEqualTo: getStringAsync(SharePreferencesKey.USER_ID))
        .snapshots()
  ...
  Future<String> addResturantInfo(Map<String, dynamic> data, {XFile? profileImage, XFile? logoImage}) async {
  ...
  Future<void> updateResturantInfo(Map<String, dynamic> data, String id, {XFile? profileImage, XFile? logoImage}) async {
  ...
  Future<void> removeCustomDocument(String id) async {
    ...
    menus.then((value) {
      value.docs.forEach((element) {
        fireStore.collection(Collections.restaurants).doc(id).collection(Collections.menus).doc(element.id).delete();
      });
    });
